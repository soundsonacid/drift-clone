...
class UpdateCurveAction(Action):
    ...
    async def execute(self, admin: Admin):
        ...
        try:
            sig = (
                await admin.repeg_curve(self.new_peg_candidate, self.market_index)
            ).tx_sig
...
class UpdateOracleAction(Action):
    ...
    async def execute(self, admin: Admin):
        ...
        print(
            f"updating oracle for market: {self.market_index} old price: {price} new price: {self.oracle_price}"
        )
        try:
...
def get_action(admin: Admin) -> Action:
    ...
    if chosen_action_class == UpdateCurveAction:
        ...
        return UpdateCurveAction(
            market_index=market_index, new_peg_candidate=new_peg_candidate
        )
        ...
        return UpdateImfAction(
            market_index=market_index,
            imf_factor=imf_factor,
            upnl_imf_factor=upnl_imf_factor,
        )
    else:
        ...
        return UpdateOracleAction(
            market_index=market_index, oracle=oracle, oracle_price=new_price
        )
