...
from main import ClosePositionEvent, SettlePnLEvent
...
class Liquidator:
    def __init__(
        ...
        self.liq_ch: DriftClient = user_chs[liquidator_index]
    ...
    async def try_liquidate_spot(self):
        ...
        for ix_args, ix in zip(ixs_args, ixs):
            promise = await self.send_ix(
                self.liq_ch, ix, "liquidate_spot", ix_args, silent_fail=self.silent
            )
    ...
    async def try_liquidate_perp(self):
        ...
        for ix in ixs:
            ...
            promise = await self.send_ix(
                self.liq_ch, ix, "liquidate_perp", silent_fail=self.silent
            )
        ...
        for ch in self.user_chs.values():
            for sid in ch.subaccounts:
                for i in range(self.n_markets):
                    ...
                    if (
                        position
                        and position.base_asset_amount == 0
                        and position.quote_asset_amount < 0
                    ):
                        promise = (
                            await self.liq_ch.get_liquidate_perp_pnl_for_deposit_ix(
                                authority,
                                i,
    ...
    async def resolve_bankruptcies(self):
        ...
        for ch, user in zip(chs, users):
            if str(user.status) == "UserStatus.Bankrupt()":
                ...
                for i in range(self.n_spot_markets):
                    ...
                    if position is not None and not is_spot_position_available(
                        position
                    ):
        ...
        for _, ix, name in zip(ix_args, ixs, ix_names):
            await self.send_ix(
                self.liq_ch, ix, name, silent_fail=False, view_logs_flag=True
            )

    async def derisk(self):
        ch: DriftClient = self.liq_ch # type: ignore
        ...
        for i in range(self.n_markets):
            ...
            market = ch.get_perp_market_account(self.liq_ch.program, i)
            slot = (await provider.connection.get_slot()).value
            liq_time = (await provider.connection.get_block_time(slot)).value
            print(
                f"=> liquidator derisking {position.base_asset_amount} baa in market status:",
                market.status,
            )

            if (
                str(market.status) == "MarketStatus.Settlement()"
                or market.expiry_ts >= liq_time
            ):
                ...
                await self.send_ix(ch, ix, SettlePnLEvent._event_name)
            else:
                print(f"=> liquidator derisking")
                ix = await ch.get_close_position_ix(i)
                # args = place_and_take_ix_args(ix[1])
                await self.send_ix(ch, ix, ClosePositionEvent._event_name)
